...
def get_absolute_path(path):
    """
        返回文件的绝对路径,如果路径包含起始点(如Unix '/')，然后使用指定的起始点,而不是当前工作目录。文件路径的起始点为,
        允许以波浪号“~”开始，它将被替换为用户的主目录。
    """
...
def get_xmind_testsuites(xmind_file):
    """
        加载 XMind 文件并解析为' xmind2testcase.metadata '。TestSuite”列表
    """
...
def get_xmind_testsuite_list(xmind_file):
    """ 加载 XMind 文件，并在其中获得所有的 testsuite
